...
//go:generate go run -mod=readonly github.com/cilium/ebpf/cmd/bpf2go -cflags="-Wunused-variable" -no-global-types sysctl ./../../ebpf/sysctl.c -- -D__TARGET_ARCH_x86 -I./../../ebpf/headers -Wall -Wno-unused-variable
...
func main() {
	...
	errg.Go(func() error {
		...
		for {
			...
			n := idx2name(i)
			log.Printf("Update %s to %d on interface %s(%d) by process %s", n, event.DevConfValue, ifname, ifindex, p)
		}
	})
