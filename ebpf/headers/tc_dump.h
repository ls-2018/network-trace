...
typedef struct event {
    ...
    u16 total_len;
#define DATA_SIZE                                                                                                      \
    (sizeof(struct ethhdr) + sizeof(struct vlan_hdr) + sizeof(struct iphdr) + sizeof(struct udphdr)                    \
        + sizeof(struct vxlan_hdr) + sizeof(struct ethhdr) + sizeof(struct iphdr) + sizeof(struct tcphdr))
    u8 data[DATA_SIZE];
#undef DATA_SIZE
...
static __always_inline bool is_ipv4_proto(__be16 proto) { return proto == bpf_htons(ETH_P_IP); }

static __always_inline int calc_l3_off(struct __sk_buff *skb)
{
    struct ethhdr *eth;
    int l3_off = 0;
...
static __always_inline bool is_vxlan_port(__be16 port) { return port == bpf_htons(VXLAN_PORT); }

static __always_inline void copy_headers(void *__skb, event_t *ev, bool is_tc)
{
    struct ethhdr *eth;
    struct vlan_hdr *vh;
    struct iphdr *iph;
