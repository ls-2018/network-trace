...
#define guard_spinlock(lock)                                                                                           \
    struct guard_spinlock_t var __cleanup(guard_spinlock_destructor) = guard_spinlock_constructor(lock)
...
#define guard_ring_buf(_ring_buf, _data, err)                                                                          \
    struct guard_ringbuf _g __cleanup(guard_ring_buf_destructor)                                                       \
        = guard_ring_buf_constructor(_ring_buf, sizeof(*_data), err);                                                  \
