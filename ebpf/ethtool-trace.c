...
static __always_inline struct event *__get_or_init_event(void)
{
...
static __always_inline struct event *__get_event(void)
{
    u32 key = 0;
    return bpf_map_lookup_elem(&events_cache, &key);
}

static __always_inline struct event *__get_and_del_event(void)
{
    struct event *ev;
    u32 key = 0;
...
static __always_inline u32 get_ethcmd(void *useraddr)
{
    u32 cmd;
