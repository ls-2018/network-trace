...
#define SOCK_PATH_OFFSET (offsetof(struct unix_address, name) + offsetof(struct sockaddr_un, sun_path))
...
static __noinline bool __is_path_matched(__u64 *path)
{
    ...
    for (i = 0; i < UNIX_PATH_MAX / 8 && sock_path[i]; i++)
        if (path[i] != sock_path[i])
            return __is_str_prefix((const char *)&path[i], (const char *)&sock_path[i], 8);

    if (i == UNIX_PATH_MAX / 8)
        return __is_str_prefix((const char *)&path[i], (const char *)&sock_path[i], 4);

    return true;
}

static __always_inline bool match_path_of_usk(struct unix_sock *usk, __u64 *path)
{
...
static __noinline int usk_send_msg(void *ctx, struct socket *sock, struct msghdr *msg, size_t len)
{
    ...
    for (int i = 0; i < SS_MAX_SEGS_PER_MSG; i++) {
        ...
        collect_data(ctx, pkt, BPF_CORE_READ(iov, iov_base), BPF_CORE_READ(iov, iov_len));
...
static __always_inline int __kprobe_unix_sendmsg(struct pt_regs *ctx)
{
